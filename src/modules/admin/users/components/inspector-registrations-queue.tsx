...
type InspectorRegistrationsQueueProps = {
  backTo: { href: string; label: string };
  lockedTenant?: { id: string; name: string };
};

export function InspectorRegistrationsQueue({
  backTo,
  lockedTenant,
}: InspectorRegistrationsQueueProps) {
  ...
  const handleApprove = async (item: InspectorRegistrationListItem) => {
    const tenantId =
      lockedTenant?.id ?? tenantSelections[item.id] ?? item.suggestedTenantId ?? "";
