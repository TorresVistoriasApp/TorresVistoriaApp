...
function FooterColumn({
  ...
  return (
    <div>
      <h3 className="text-[11px] font-bold uppercase tracking-[0.09em] text-ink-muted">{title}</h3>
  ...
  return (
    <footer className="bg-ink text-ink-foreground">
      <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-14 lg:px-8">
            ...
            <div className="mt-5 flex gap-2">
              {SOCIAL.map(({ icon: Icon, label, href }) => (
                <a
                  ...
                  href={href}
                  aria-label={label}
                  className="flex h-9 w-9 items-center justify-center rounded-lg border border-ink-border text-ink-muted transition-colors hover:border-primary/50 hover:text-primary"
                >
        ...
        <div className="mt-11 flex flex-col gap-3 border-t border-ink-border pt-6 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-ink-muted">
            © {new Date().getFullYear()} Torres Consulta. Todos os direitos reservados.
          </p>
          <p className="text-xs text-ink-muted">
            Parte do Ecossistema Torres. Conheça também a{" "}
            <Link to={ROUTES.vistoriaLogin} className="font-semibold text-ink-foreground hover:text-primary">
              Torres Vistoria
            </Link>
          </p>
        </div>
