    ...
    <div className="-mx-1 overflow-x-auto px-1 pb-1">
      <div className="flex min-w-min gap-2 sm:gap-2.5">
        <button
          type="button"
          onClick={onAdd}
          className="flex w-[7.5rem] shrink-0 flex-col overflow-hidden rounded-xl border-2 border-dashed border-brand-border bg-brand-subtle transition-colors duration-150 hover:border-primary hover:bg-brand-muted sm:w-[8.5rem]"
        >
        ...
        {sorted.map((photo, index) => {
          ...
          return (
            <button
              ...
              disabled={uploading}
              className={cn(
                "relative w-[7.5rem] shrink-0 overflow-hidden rounded-xl border bg-card text-left shadow-soft transition-shadow duration-150 hover:shadow-elevated sm:w-[8.5rem]",
                uploading ? "border-border opacity-70" : "border-border",
              )}
            >
              <span className="relative block aspect-[4/3] w-full bg-muted">
                ...
                {!uploading && (
                  <span className="absolute right-1 top-1 flex size-5 items-center justify-center rounded-full bg-success text-white shadow-soft">
                    <CheckCircle2 className="size-3" aria-hidden />
                  </span>
                )}
                {uploading && (
                  <span className="absolute inset-0 flex items-center justify-center bg-canvas/80">
                    <span className="size-5 animate-spin rounded-full border-2 border-primary border-t-transparent" />
                  </span>
                )}
              </span>
              <span className="block border-t border-border px-1.5 py-1.5">
